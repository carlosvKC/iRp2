...
- (id)initWithFontSize:(CGFloat)size withPad:(int)pad
    {
        if (self = [super initWithFrame:CGRectMake(0,0,16,16)]) 
        {
            // Initialization code
...
-(CGSize)badgeSize
{
	...
	badgeRect.origin.x = 0;
	badgeRect.origin.y = 0;
	badgeRect.size.width = ceil( badgeRect.size.width + 2.0);
	badgeRect.size.height = ceil( badgeRect.size.height + 2.0);
