...
- (void)drawRect:(CGRect)rect {
	...
	//CGContextDrawImage(ctx, self.bounds, [self drawToImage:rect].CGImage);
...
- (UIImage*)drawToImage: (CGRect)rect
{
    if (!_valid)
    {
        ...
        int x = CGRectGetMidX(rect);// self.center.x;
        int y = CGRectGetMidY(rect);
        int r = (rect.size.width>rect.size.height?rect.size.height:rect.size.width)/2 * 0.8;
        ...
        // (1)
        UIImage *gradientImage = [self createGradientImageUsingRect: rect];
