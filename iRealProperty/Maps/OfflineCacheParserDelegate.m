...
- (void)parser:(NSXMLParser *)parser didEndElement:(NSString *)elementName namespaceURI:(NSString *)namespaceURI qualifiedName:(NSString *)qName
{
    ...
    if ([elementName isEqualToString:@"LODInfo"]){
        ...
        // added two artificial levels to allow go deeper
        /* if(self.lod != nil)
        {
            self.lod = [[AGSLOD alloc]initWithLevel:self.lod.level + 1 resolution:self.lod.resolution / 2.0 scale:self.lod.scale / 2.0];
            [self.lods addObject:_lod];
            self.lod = [[AGSLOD alloc]initWithLevel:self.lod.level + 1 resolution:self.lod.resolution / 2.0 scale:self.lod.scale / 2.0];
            [self.lods addObject:_lod];
            self.lod = [[AGSLOD alloc]initWithLevel:self.lod.level + 1 resolution:self.lod.resolution / 2.0 scale:self.lod.scale / 2.0];
        ...
        */
        ...
        {
            // AGSLOD *lod = [_lods objectAtIndex:i];
            //    NSLog(@"%d, res=%f scale=%f", lod.level, lod.resolution, lod.scale);
        }
